...
import com.demo.ui.uiapplication.materialdesignAnimation.activityAnimation.AnimationFirstActivity;
import com.demo.ui.uiapplication.materialdesignAnimation.activityAnimation.AnimationThirdActivity;
import com.demo.ui.uiapplication.materialdesignAnimation.svg.SVGTest;
 ...
 */
public class MdAnimationTest extends FragmentActivity implements View.OnClickListener {
    View view_revelShow;
    View view_revelShow2;
    ClipView clipView;
    int revelWidth;
    int revelHeight;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.md_animation_test);
        view_revelShow = findViewById(R.id.tv_revel_show);
        view_revelShow2 = findViewById(R.id.tv_revel_show_2);
        clipView = findViewById(R.id.clip_view);
        view_revelShow.post(new Runnable() {
            @Override
            public void run() {
                revelWidth = view_revelShow.getWidth();
                revelHeight = view_revelShow.getHeight();
            }
        });
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.fl_valueAnimator:
                Intent intent = new Intent(this, AnimationThirdActivity.class);
                startActivity(intent);
                break;
            case R.id.tv_ripple:
                intent = new Intent(this, AnimationFirstActivity.class);
                startActivity(intent);
                break;
            case R.id.tv_svg:
                intent = new Intent(this, SVGTest.class);
                startActivity(intent);
                break;
            case R.id.tv_revel:
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    Animator animator = ViewAnimationUtils.createCircularReveal(view_revelShow, revelWidth / 2, revelHeight / 2, 0, (float) Math.hypot(revelWidth / 2, revelHeight / 2));
                    animator.setDuration(1000);
                    animator.setInterpolator(new AccelerateInterpolator());
                    animator.addListener(new AnimatorListenerAdapter() {
                        @Override
                        public void onAnimationStart(Animator animation) {
                            if (view_revelShow.getVisibility() != View.VISIBLE)
                                view_revelShow.setVisibility(View.VISIBLE);
                        }
                    });
                    animator.start();


                    animator = ViewAnimationUtils.createCircularReveal(view_revelShow2, revelWidth, 0, 0, (float) Math.hypot(revelWidth, revelHeight));
                    animator.setDuration(1000);
                    animator.setInterpolator(new AccelerateInterpolator());
                    animator.addListener(new AnimatorListenerAdapter() {
                        @Override
                        public void onAnimationStart(Animator animation) {
                            if (view_revelShow2.getVisibility() != View.VISIBLE)
                                view_revelShow2.setVisibility(View.VISIBLE);
                        }
                    });
                    animator.start();
                }

                //属性动画+PorterDuff.Mode.DST_OUT实现revel效果
                ...
                valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animation) {
                        clipView.setRevelLevel((float) animation.getAnimatedValue());
                    }
                });
