package com.baza.android.slib.network.http;

import android.text.TextUtils;
import android.util.Log;

import com.baza.android.slib.storage.file.FileManager;
import com.baza.android.slib.utils.string.MD5;
...
public class FileLoadTool {
    ...
    public void downLoadFile(String url, String extensionName, String tagForSameUrl) {
        downLoadFile(url, null, extensionName, tagForSameUrl);
    }

    public void downLoadFile(String url, String fileSaveDir, String extensionName, String tagForSameUrl) {
    ...
    public void downLoadFile(String url, String fileSaveDir, String extensionName, String tagForSameUrl, HashMap<String, String> header) {
        ...
        File fileAlreadyDownLoad = getDownFile(fileSaveDir, fileName, false);
        if (fileAlreadyDownLoad != null) {
            //文件已经下载
        ...
        HttpRequestUtil.downLoadFile(url, header, 120000, 30000, new FileLoadResultCallBack<String>(new FileLoadParam(url, fileSaveDir, fileName, tagForSameUrl), url, String.class, HttpRequestUtil.getHttpConfig()) {
            @Override
    ...
    public File getExistsDownLoadFileByUrl(String fileSaveDir, String url, String tagForSameUrl) {
        String fileName = getFileNameByUrl(url, null, tagForSameUrl);
        return getDownFile(fileSaveDir, fileName, false);
    }

    public File getExistsDownLoadFileByUrl(String url) {
        return getExistsDownLoadFileByUrl(null, url, null);
    }

    /**
     * 获取下载的附件
     *
     * @param fileSaveDir  文件存储路径
     * @param fileName     文件名称
     * @param isAutoCreate 文件不存在时是否自动创建
     * @return 下载的附件
     */
    public static File getDownFile(String fileSaveDir, String fileName, boolean isAutoCreate) {
        if (!TextUtils.isEmpty(fileSaveDir))
            return FileManager.getFile(fileSaveDir, fileName, null, isAutoCreate);

        return FileManager.getDownLoadFile(fileName, isAutoCreate);
    }
    ...
    public static File getTempFile(String fileName, boolean isAutoCreate) {
        return FileManager.getTempFile((fileName + ".temp"), isAutoCreate);
    }
